...
// Import Hardhat plugin type augmentations
...
export async function sendEvm(
  ...
  if (oftAddress) {
    ...
    wrapperAddress = wrapper.contract.contractName
      ? (await (srcEidHre as any).deployments.get(wrapper.contract.contractName)).address
  ...
  const amountUnits: BigNumberish = parseUnits(amount, decimals);
